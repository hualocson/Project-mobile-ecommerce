...
class SaleAdminAdapter(
    private val context: Context,
    private val onClick: (SaleJson) -> Unit,
) : RecyclerView.Adapter<SaleAdminAdapter.NewViewHolder>(), BindableAdapter<SaleJson> {
    private var items: ArrayList<SaleJson> = ArrayList()

    inner class NewViewHolder(private val binding: ItemSalesAdminBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bindData(saleJson: SaleJson) {
            binding.newData = saleJson
            binding.executePendingBindings()
            binding.btnUpdate.setOnClickListener { onClick(saleJson)}
        }
    }
    ...
    override fun onCreateViewHolder(
        ...
        val binding =
            ItemSalesAdminBinding.inflate(LayoutInflater.from(context), parent, false)
